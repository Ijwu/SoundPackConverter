...
using SoundPackConverter.ConfigEditor.WPF;

namespace SoundPackConverter.ConfigEditor
{
    /// <summary>
    /// Maps a json file to a <![CDATA[ List<Node> ]]> .
    /// </summary>
    public class SoundMappingTreeConverter : JsonConverter
    {
        ...
        private Node HandleJProperty(JProperty jProperty)
        {
            var currentNode = new Node(jProperty.Name);
        ...
        private Node HandleJObject(JObject jObject, string key)
        {
        ...
        private Node HandleJArray(JArray jArray, string key)
        {
            var root = new Node(key);
            foreach (JToken token in jArray)
            {
                if (token.Type == JTokenType.String)
                {
                    ...
                    if (jvalue == null)
                        continue;
                    root.Nodes.Add(new Node(jvalue.Value<string>()));
                }
            }
            return root;
        }
